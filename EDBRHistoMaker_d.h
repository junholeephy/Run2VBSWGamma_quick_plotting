...
/// EDBRHistoMaker_d is the class that analyzes the flat
...
class EDBRHistoMaker_d {
 public:
  EDBRHistoMaker_d(TTree *tree=0,
         TFile *fileTMP=0,
                 TH1F* hR1=0,
...
void EDBRHistoMaker_d::Init(TTree *tree)
{
...
EDBRHistoMaker_d::EDBRHistoMaker_d(TTree* tree,
                               TFile* fileTMP,
                               TH1F* hR1,
			       bool wantElectrons,
			       bool wantMuons,
...
EDBRHistoMaker_d::~EDBRHistoMaker_d() {
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}                  

Int_t EDBRHistoMaker_d::GetEntry(Long64_t entry) {
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}

Long64_t EDBRHistoMaker_d::LoadTree(Long64_t entry) {
...
void EDBRHistoMaker_d::createAllHistos() {
...
void EDBRHistoMaker_d::printAllHistos() {
...
void EDBRHistoMaker_d::saveAllHistos(std::string outFileName) {
...
///----------------------------------------------------------------
void EDBRHistoMaker_d::Loop(std::string outFileName){
